...
Auto-generated. Stores x/y positions for the TUI canvas. Can be gitignored per team preference — if absent, tngl uses an auto-layout algorithm on first open.

```
# Auto-generated by tngl — safe to gitignore
src/main.rs       x:120  y:340
src/auth/         x:280  y:210
docs/arch.md      x:450  y:380
```
...
- Generates `layout.tngl` with auto-layout positions
...
- Updates `layout.tngl` with positions for new nodes
...
`tngl init` appends to `.gitignore`:
...
The layout file is committed by default but the comment signals teams can opt out.
...
tngl/
  src/
    ...
    parser/
      ...
      layout.rs      # layout.tngl parser and writer
    scanner/
    ...
    html/
      export.rs      # static graph.html generator
  Cargo.toml
```
